...
-(instancetype)init{
    if(self = [super init]){
        ...
        _mTextBtn.bounds = CGRectMake(0, 0, SSChatTextWidth2, SSChatTextHeight);
        ...
        SSChatKeyBordVoiceView *voiceView = [[SSChatKeyBordVoiceView alloc] initWithFrame:_mKeyBordView.bounds];
        [_mKeyBordView.customView addSubview:voiceView];
...
-(UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event{
    ...
    if(point.y>SSChatKeyBoardInputViewH){
        ...
        else if(_mKeyBordView.type == KeyBordViewFouctionCustom){
            for(UIView * view in _mKeyBordView.customView.subviews){
                [array addObjectsFromArray:view.subviews];
            }
        }
    ...
    switch (self.keyBoardStatus) {
        ...
        case SSChatKeyBoardStatusDefault:{
            if(sender.tag==10){
                ...
                self.currentBtn = sender;
                self.currentBtn.selected = YES;
                _mKeyBordView.type = KeyBordViewFouctionCustom;
                _mKeyBordView.mCoverView.hidden = YES;
                self.keyBoardHieght = SafeAreaBottom_Height+SSChatKeyBordHeight;
                [self setNewSizeWithBootm:_textH];
            }
...
- (void)beginRecordVoice:(UIButton *)button{
    ...
    cout(@"beginRecordVoice");
}

- (void)endRecordVoice:(UIButton *)button{
    cout(@"endRecordVoice");
    // 模拟录音完成：空数据
    [self recordAudio:nil didCompletedWithError:nil];
}
...
- (void)cancelRecordVoice:(UIButton *)button{
    cout(@"cancelRecordVoice");
    // 模拟已取消
    [self recordAudioDidCancelled];
}
...
- (void)recordAudioInterruptionBegin {
    cout(@"recordAudioInterruptionBegin");
    // should cancelRecord
}
